...
export class CanvasComponent implements OnInit {
  ...
  drawArea() {
    ...
    // circle
    this.ctx.beginPath();
    this.ctx.arc(0, 0, halfRadius, -Math.PI, Math.PI/2, true);
    this.ctx.lineTo(0, 0);
    this.ctx.fill();
    // rectangle
    this.ctx.beginPath();
    this.ctx.rect(-halfRadius, -2*halfRadius, halfRadius, this.radiusDrawn);
    this.ctx.fill();
    // triangle
    this.ctx.beginPath();
    this.ctx.moveTo(0, 0);
    this.ctx.lineTo( this.radiusDrawn, 0);
    this.ctx.lineTo(0, -halfRadius);
    this.ctx.lineTo(0, 0);
    this.ctx.fill();
    // axes
    this.ctx.beginPath();
    this.ctx.fillStyle = "#826ffa";
